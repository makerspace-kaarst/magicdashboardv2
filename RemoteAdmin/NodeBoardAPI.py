...
def upload_file(filename):
    return requests.post(IP + ':' + str(PORT) + '/upload_file', data=open(filename, 'rb'),
                         headers={'filename': filename, 'X-API-Auth': PASSWORD}).text


# Delete a file from the NodeBoard Server
def delete_file(filename):
    return requests.post(IP + ':' + str(PORT) + '/delete_file',
                         headers={'filename': filename, 'X-API-Auth': PASSWORD}).text
...
def unsave_console():
    print(
        'API functions can be used without module name, you are authenticated with the data enered by whatever dropped you here.\n\n')
    while True:
        try:
            cmd = input('>>> ')
            if cmd == 'exit':
